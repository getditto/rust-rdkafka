...
/// The result of a [`FutureProducer::queue_batch`] call.
///
/// Queuing records and awaiting their delivery confirmations are intentionally
/// separated so callers can:
///
/// 1. Run the (potentially blocking) enqueue loop on a dedicated thread via
///    [`tokio::task::spawn_blocking`], bounded by a Tokio timeout.
/// 2. Await delivery callbacks in an async context — without holding a blocking
///    thread — once all records are in librdkafka's queue.
///
/// # Example
///
/// ```no_run
...
/// # async fn example(producer: FutureProducer) {
/// // Owned data that can be moved into spawn_blocking.
...
///         let records = vec![FutureRecord::to(&topic).payload(&payload)];
///         producer2.queue_batch(records, Timeout::Never, true)
///     }),
/// )
/// .await
/// .expect("queue timed out")
/// .expect("queue_batch panicked");
///
/// let results = queued.await_results().await;
/// # }
/// ```
pub struct QueuedBatch {
    rxs: Vec<oneshot::Receiver<OwnedDeliveryResult>>,
    enqueue_err: Option<OwnedDeliveryResult>,
}

impl QueuedBatch {
    /// Wait for every queued message to be acknowledged (or rejected) by the broker.
    ///
    /// Results are returned in submission order. On the first delivery error the
    /// method stops early and returns a prefix of successful results followed by
    /// the error, matching the behaviour of [`FutureProducer::send_batch`].
    pub async fn await_results(self) -> Vec<OwnedDeliveryResult> {
        let mut results =
            Vec::with_capacity(self.rxs.len() + self.enqueue_err.is_some() as usize);
        for rx in self.rxs {
        ...
        if let Some(enqueue_err) = self.enqueue_err {
    ...
    R: AsyncRuntime,
{
    ...
    /// Synchronously enqueues a batch of records into the producer's internal queue,
    /// returning a [`QueuedBatch`] whose delivery results can be awaited later.
    ///
    /// Unlike [`send_batch`], this method is **purely synchronous**: it uses
    /// [`std::thread::sleep`] for back-off when the librdkafka queue is full, so it
    /// **must not** be called directly from an async context. Use
    /// [`tokio::task::spawn_blocking`] (or an equivalent blocking-thread executor)
    /// to run it, and wrap the call with [`tokio::time::timeout`] to bound the
    /// enqueue time.
    ///
    /// Once the returned [`QueuedBatch`] is in hand, call
    /// [`QueuedBatch::await_results`] from an async context to collect delivery
    /// confirmations without holding a blocking thread.
    ///
    /// The `queue_timeout` parameter controls how long to retry when the queue is
    /// full. Set it to [`Timeout::Never`] to retry indefinitely, or
    /// [`Timeout::After(Duration::ZERO)`] to fail immediately on a full queue.
    ///
    /// If `force_flush` is `true`, a non-blocking flush is triggered after
    /// enqueuing, bypassing the delay imposed by `linger.ms`.
    ///
    /// [`send_batch`]: FutureProducer::send_batch
    pub fn queue_batch<K, P, T>(
        &self,
        records: Vec<FutureRecord<'_, K, P>>,
        queue_timeout: T,
        force_flush: bool,
